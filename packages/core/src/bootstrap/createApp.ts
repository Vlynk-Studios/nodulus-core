...
import { generateTsConfigNodulus } from '../aliases/tsconfig-generator.js';
...
export async function createApp(
  ...
  // Step 1.0.5 — Alias Conflict Validation
  if (config.aliases && (config.aliases['@modules'] || config.aliases['@modules/*'])) {
    throw new NodulusError(
      'ALIAS_CONFLICT',
      'The alias "@modules" is reserved and auto-generated by Nodulus.',
      'Remove "@modules" from your nodulus.config.ts aliases.'
    );
  }

  // Step 1.0.6 — Auto-generate tsconfig.nodulus.json
  await generateTsConfigNodulus(config, process.cwd(), log);
